    ...
    const fetchAll = async () => {
      try {
        ...
        const [
          ...
          portfolioApi.getProjects({ featured: true, limit: 300 }),
...
//   }, [names.length, profiles.length]);
  ...
  //         <p style={{ color: colors.textPrimary }}>Loading projects...</p>
  //       </div>
  //     </div>
  //   );
  // }
          ...
          return rows.map((row, rowIndex) => (
            <div key={rowIndex} className={styles.projectRow}>
              {row.map((project, index) => {
                // Alternate: first card wide (2/3), second narrow (1/3)
                ...
                return (
                  <motion.div 
                    ...
                    viewport={{ once: true }} 
                    transition={{ duration: 0.5, delay: (rowIndex * 2 + index) * 0.1 }} 
